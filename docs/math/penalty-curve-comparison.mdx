...
The hook uses a `frac^1.5` penalty curve. This page walks through the other curve shapes that were considered and explains why they were rejected. It's a useful reference if you want to fork the hook and use a different curve, for example one that punishes aggressive bidders more harshly.
...
We wrote down a target reference table:
...
- **`frac^1.5`** is smooth, matches the target penalty shape, and is still cheap on-chain (see [Penalty Curve](/math/penalty-curve)).