...
<Frame>
  ![Frame 74](/images/Frame-74.png "Frame 74")
</Frame>

**`src/MPFHook.sol`** — the Uniswap v4 hook itself. Holds all contract storage and implements the `BaseHook` callbacks (`_afterInitialize`, `_beforeSwap`, `_afterSwap`); contains no math of its own, only orchestration that feeds data into the libraries below.
...
**`src/lib/PenaltyFeeLibrary.sol`** — pure math for the penalty curve plus its constants (including `BASIC_FEE`); given a swap's priority fee and the reference median, returns the LP fee to charge.
...
<Frame>
  ![Frame 75](/images/Frame-75.png "Frame 75")
</Frame>

**`test/unit/PenaltyFeeLibrary.t.sol`** — isolates `getDynamicFee_`, checking the penalty curve's shape: no fee below threshold, monotonic increase, and saturation at the max.
